...
import { Cielo, CieloConstructor, TransactionCreditCardRequestModel, EnumBrands, CaptureRequestModel, CancelTransactionRequestModel } from 'cielo';
...
export const authorizeCreditCard = functions.https.onCall(async (data, context) => {
    ...
    console.log('iniciando a autorização');
...
export const newOrder = functions.firestore.document("/orders/{orderId}").onCreate(async (snapshot, context) => {
    const orderId = context.params.orderId;

    const querySnapshots = await admin.firestore().collection('admins').get();

    const admins = querySnapshots.docs.map(doc => doc.id);
    ...
    await sendPushFCM(adminTokens, 'Novo Pedido', 'Nova venda realizada. Pedido: ' + orderId)
...
export const onOrderStatusChange = functions.firestore.document("/orders/{orderId}").onUpdate(async (snapshot, context) => {
    ...
    if (beforeStatus !== afterStatus) {
        ...
        await sendPushFCM(tokensuser, 'Pedido:' + context.params.orderId, 'Status atualizado para:' + orderStatus.get(afterStatus));
    }
});

async function getDeviceTokens(uid: string) {
    const querySnapshots = await admin.firestore().collection('users').doc(uid).collection('tokens').get();

    const tokens = querySnapshots.docs.map(doc => doc.id);
